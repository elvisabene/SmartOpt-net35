...
namespace SmartOpt
{
    public static class Program
    {
        ...
        private static void HandleArguments(string[] args)
        {
            ...
            var aggregatedArgs = args.Aggregate((s1, s2) => $"{s1} {s2}");
            ...
            if (argumentsParser.TryParseGuiType(aggregatedArgs, out var guiType))
            {
                ApplicationState.SetGuiType(null!, guiType);
            }

            if (!argumentsParser.TryParseOperationType(aggregatedArgs, out var operationType))
            {
                return;
            }
            ...
            if (argumentsParser.TryParseExcelWorkbookFilepath(aggregatedArgs, out var workbookFilepath))
            {
                ApplicationState.SetExcelWorkbookFilepath(null!, workbookFilepath);
            }

            if (argumentsParser.TryParseGroupSize(aggregatedArgs, out var groupSize))
            {
            ...
            if (argumentsParser.TryParseMaxWidth(aggregatedArgs, out var maxWidth))
            {
            ...
            if (argumentsParser.TryParseMaxWaste(aggregatedArgs, out var maxWaste))
            {
